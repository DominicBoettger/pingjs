...
import pingoptions = require('./types/pingoptions');
...
class pinger {
  public ping(dest, options):Promise<any> {
      ...
      return new Promise((resolve, reject) => {
          ...
          function sendPing () {
            ...
            socket.send(buffer, 0, buffer.length, dest,
              function (err) {
                if (err) {
                  reject(err.toString());
                }
  ...
  protected generateEchoId() {
    return crypto.randomBytes(2);
  }
  protected pingMessage(isReply, pingId, sequence, data) {
      if( data.length % 2 === 0) {
        data = data;
      } else {
        data = data + '\x00';
